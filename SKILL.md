...
- **번호 복사 블록.** 교수님이 그 번호로 다시 말할 가능성이 큰 답변에는 본문 **끝에** 번호를 한 줄짜리 코드 블록으로 **하나씩 따로** 붙인다.
  Discord가 코드 블록마다 "복사" 버튼을 붙여 주므로(데스크톱·웹은 마우스를 올리면 오른쪽 위, 모바일은 블록을 누르면) 클릭 한 번으로 클립보드에 들어간다.
  - 대상: 등록 결과(새 `CLR-`), 새 질문(`Q-`), 후보 되묻기(`ambiguous_item`·`ambiguous_question`의 각 후보), 중복 안내(`existing_items`), 승인 요청(`requires_confirm`), `show`·`find` 결과의 항목.
  - 형식: 본문 한 줄 띄고 `` ```CLR-0031``` `` 처럼 여는 백틱 셋·번호·닫는 백틱 셋을 **한 줄에**. 블록 하나에 번호 하나(여럿을 넣으면 한꺼번에 복사된다). 설명·괄호·마침표를 블록 안에 넣지 않는다.
  - 한 메시지에 5개까지. 넘치면 교수님이 부를 만한 순서(새 항목 → 질문 → 후보)로 자르고 "나머지 번호는 `CLR-0031 번호`라고 하면 드립니다"를 붙인다.
  - `CLR-0031 번호` / `Q-0007 번호` / `번호 CLR-0031`이라고 하면 그 번호의 제목 한 줄과 복사 블록만 답한다(DB 무변경).
  - 브리핑은 `references/briefing-format.md`의 "바로 답할 번호" 규칙을 따른다. 그 외 짧은 대화·조회 결과(`today`/`week`/`overdue`)에는 붙이지 않는다.
  - 예(바깥 4중 백틱은 문서용, 실제 답변은 안쪽 내용만):
    ````
    ○○ 교수 면담(카카오톡)을 CLR-0031로 등록했습니다. 시각이 없어 Q-0007로 여쭙습니다: 금요일 오후 몇 시인가요? 후보 14:00 / 15:00 / 16:00
